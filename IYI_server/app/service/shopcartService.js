  ...
  async getCartData(uid, gid, count) {
    if (gid && count) {
      ...
      if (queryResult.length) {
        ...
        // let testSql = `select * from shopcart where id = ? and goodsid=?`
        // let testR = await this.ctx.app.mysql.query(testSql, [uid, gid]);
...
values(?,?,?,?)`
    ...
    let getCartDataSql = `select goods.id,goods.name,goods.price,goods.cover,goods.rest,shopcart.count from user join shopcart on user.id = shopcart.userid join goods on shopcart.goodsid = goods.id where user.id=? and shopcart.state = 0;`;
